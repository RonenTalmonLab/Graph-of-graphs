...
if plt_sensorM == 1
    ...
    %     line([0,401],[4001,2001],'linewidth',2,'color','k')
    end
...
Psi = cell(1,1);
dist = squareform(pdist(y.','minkowski',1)); 
...
M_mea_S3 = squeeze(cat(1,M_mea{:}))';
C_mea_S3 = squeeze(cat(1,C_mea{:}))';

SSD = cat(1,SSD{:});

dist_SSD = squareform(pdist(SSD,'minkowski',1)); 
W = exp(-dist_SSD.^2/(10*median(dist_SSD(:))^2));
W = diag(1./sum(W,1))*W;
[PsiSSD,~] = eigs(W,4);
PsiSSD = PsiSSD(:,2:4);
%%
SSD1 = cell(1,1);
for pos2 = 1:snum*2
    distSSD = squareform(pdist(measure_y{pos2}(:,2:12)')); 
    W = exp(-distSSD.^2/(median(distSSD(:))^2));
    SSD1{pos2} = sum(W)/sum(sum(W)); 
end
SSD1 = cat(1,SSD1{:});

dist_SSD = squareform(pdist(SSD1,'minkowski',1));
W = exp(-dist_SSD.^2/(10*median(dist_SSD(:))^2));
W = diag(1./sum(W,1))*W;
[PsiSSD_1,~] = eigs(W,4);
PsiSSD_1 = PsiSSD_1(:,2:4);

SSD7 = cell(1,1);
for pos2 = 1:snum*2
    distSSD = squareform(pdist(cat(2,measure_y{pos2}(:,2:6),measure_y{pos2}(:,8:12))')); 
    W = exp(-distSSD.^2/(median(distSSD(:))^2));
    SSD7{pos2} = sum(W)/sum(sum(W)); 
end
SSD7 = cat(1,SSD7{:});

dist_SSD = squareform(pdist(SSD7,'minkowski',1)); 
W = exp(-dist_SSD.^2/(10*median(dist_SSD(:))^2));
W = diag(1./sum(W,1))*W;
[PsiSSD_7,~] = eigs(W,4);
PsiSSD_7 = PsiSSD_7(:,2:4);

SSD17 = cell(1,1);
for pos2 = 1:snum*2
    distSSD = squareform(pdist(cat(2,measure_y{pos2}(:,1:6),measure_y{pos2}(:,8:12))')); 
    W = exp(-distSSD.^2/(median(distSSD(:))^2));
    SSD17{pos2} = sum(W)/sum(sum(W)); 
end
SSD17 = cat(1,SSD17{:});
dist_SSD = squareform(pdist(SSD17,'minkowski',1)); 
W = exp(-dist_SSD.^2/(10*median(dist_SSD(:))^2)); 
W = diag(1./sum(W,1))*W;
[PsiSSD_17,~] = eigs(W,4);
PsiSSD_17 = PsiSSD_17(:,2:4);

%%

inpts = [{PsiSSD},{PsiSSD_17},Psi];
titles = {'\pi_{i}','\pi_{i}^{1,7}','Concatenated','Sensor 1','Sensor 2','Sensor 3','Sensor 4','Sensor 5',...
    'Sensor 6','Sensor 7','Sensor 8','Sensor 9','Sensor 10',...
    'Sensor 11','Sensor 12'};
